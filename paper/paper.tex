...
\begin{figure*}[t]
\centering
  \includegraphics[clip, trim = 3cm 2.4cm 3cm 2.1cm, width=\linewidth, height = 0.7\columnwidth]{Figs/Bases.eps}
  \caption{Decoder filters obtained by training the CAE on magnitude-spectrograms of utterances of a male speaker. This decomposition is obtained for the configuration $K = 80$ and $T = 8$. We see that the filters resemble snippets of a speech spectrogram.}~\label{fig:cnn_demo_speech}
\end{figure*}
...
\begin{figure*}
  ...
  \begin{tikzpicture}[auto, node distance=2cm,>=latex']
      ...
      \node [,name=input] (input) {$X$};
      \coordinate [, name=rinput, right of=input, node distance=2cm ](rinput) {};
	
      \node [, right of=rinput] (controller) {$\vdots$};
      \node [block, above of=controller,node distance=1.3cm] (up){$\mathbf W^\ddagger_1$};
      \node [block, below of=controller,node distance=1.3cm] (rate) {$\mathbf W^\ddagger_K$};
      \node [sum, right of=controller,node distance=2cm] (sum2) {CT};
...
\begin{figure*}[ht!]
\centering
  \includegraphics[clip, trim = 0cm 0cm 0cm 0cm, width=0.95\linewidth]{Figs/CNN_Separation_Performance.png}
