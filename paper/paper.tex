...
As we have discussed above, the encoder part acts as the inverse filter to the decoder. We can think of a convolutional networks as a filter with a finite impulse response (FIR). As known from DSP theory, the inverse of an FIR filter is an IIR filter. Therefore, we explore using a Recurrent Neural Network to get an IIR encoder. The encoder equation is obtained as follows: 
